...
const Items = ({name, category, amount, image, count, setPrev, item}) => {
    ...
    <ItemStyle className='flex m-[.5rem] bg-[#f2f2f2] items-center gap-[1rem] p-[.5rem] justify-between'>
        ...
        <p className='w-[30%] text-[1.5rem]'><span className='cursor-pointer' onClick={Decrease}>-</span><span className='font-semibold mx-[1rem] border-[1px] border-[solid] p-[.5rem]'>{item.count}</span><span className='cursor-pointer' onClick={Increase}>+</span></p>
        <img src={image} alt="" className='h-[80px] w-[80px]'/>
        <p className='w-[30%] text-[1.2rem] text-center'>${(amount*count).toFixed(2)}</p>
